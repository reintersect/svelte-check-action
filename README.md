...
| `tsgo` | Hand TypeScript diagnostics to tsgo (TypeScript 7, native) instead of the TypeScript JS API. Requires a tsconfig/jsconfig and `@reintersect/effect-tsgo` or `@typescript/native` in the workspace; runs `@reintersect/svelte-check@^4.8.1` and writes generated files to `node_modules/.cache/svelte-lsp` in each package that has components. | `false` |
